...
function sonicPaymentCls(status: string | null) {
  if (!status) return "text-muted-foreground border-border";
  const s = status.toLowerCase();
  if (s.startsWith("payment -") || s === "paid") return "text-emerald-600 border-emerald-200 bg-emerald-50 dark:bg-emerald-950/30 dark:border-emerald-800 dark:text-emerald-400";
...
function OrderDrawer({
  ...
  return (
    <Dialog open={!!order} onOpenChange={open => !open && onClose()}>
      <DialogContent className="max-w-5xl w-[90vw] h-[90vh] flex flex-col p-0 gap-0 overflow-hidden [&>button:last-child]:hidden">
        {/* Header */}
        <DialogHeader className="px-5 py-4 border-b shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                <DialogTitle className="text-[15px]">{order?.orderNumber ?? "—"}</DialogTitle>
                {order && (
                  <>
          ...
          <ScrollArea className="flex-1">
            <div className="p-5">
              ...
              <div className="grid grid-cols-[1fr_340px] gap-5">
                ...
                <div className="space-y-4">
                  <Card>
                    ...
                    <CardContent className="p-0">
                      ...
                      <div className="border-t px-4 py-3 space-y-1.5 text-xs bg-muted/20">
                        {subtotal > 0 && subtotal !== orderTotal && (
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">Subtotal</span>
                            <span className="tabular-nums">{fmtGBP(subtotal)}</span>
                          </div>
                        )}
                ...
                {/* Right: Customer, shipping, courier */}
                <div className="space-y-4">
                  {/* Customer */}
                  <Card>
                    <CardContent className="p-4">
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-3">Customer</p>
                      ...
                      <div className="space-y-1.5 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1.5"><Hash size={11} /> {ch.label}</span>
                        <span className="flex items-center gap-1.5"><Banknote size={11} /> {fmtGBP(orderTotal)}</span>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-4">
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-3">Ship to</p>
                    ...
                    <Card>
                      ...
                      <CardContent className="px-4 pb-4 space-y-2.5 text-xs">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-mono font-medium text-[11px]">{order.trackingNumber}</span>
                        </div>
                        {trackingData ? (
                          <>
                            {/* Status row */}
                            ...
                            {(() => {
                              const weightChg  = trackingData.shipping_charges ?? 0;
                              const fuel       = trackingData.fuel_surcharge ?? 0;
                              const gst        = trackingData.gst ?? 0;
                              const wht        = trackingData.wht ?? 0;
                              const codSst     = trackingData.cod_sst ?? 0;
                              const codAmt     = trackingData.cod_amount ?? 0;
                              const isCod      = codAmt > 0;
                              const hasCharges = trackingData.shipping_charges != null;
                              const netReceivable = hasCharges
                                ? codAmt - wht - codSst - weightChg - fuel - gst
                                : null;
                              return (
                                <div className="mt-1 border-t pt-3 space-y-1.5 text-xs">
                                  {/* COD collected */}
                                  {isCod && (
                                    <div className="flex items-center justify-between">
                                      <span className="text-muted-foreground">COD collected</span>
                                      <span className="tabular-nums font-semibold">{fmtPKR(codAmt)}</span>
                                    </div>
                                  )}
                                    ...
                                    <div className="space-y-1 border-t pt-2 mt-1">
                                      {weightChg > 0 && (
                                        <div className="flex justify-between text-muted-foreground">
                                          ...
                                          <span className="tabular-nums">− {fmtPKR(weightChg)}</span>
                                        </div>
                                      )}
                                      {fuel > 0 && (
                                        <div className="flex justify-between text-muted-foreground">
                                          <span>Fuel surcharge</span>
                                          <span className="tabular-nums">− {fmtPKR(fuel)}</span>
                                        </div>
                                      )}
                                      {gst > 0 && (
                                        <div className="flex justify-between text-muted-foreground">
                                          <span>GST</span>
                                          <span className="tabular-nums">− {fmtPKR(gst)}</span>
                                        </div>
                                      )}
                                      {wht > 0 && (
                                        <div className="flex justify-between text-muted-foreground">
                                          <span>WHT (2%)</span>
                                          <span className="tabular-nums">− {fmtPKR(wht)}</span>
                                        </div>
                                      )}
                                      {codSst > 0 && (
                                        <div className="flex justify-between text-muted-foreground">
                                          <span>COD SST (2%)</span>
                                          <span className="tabular-nums">− {fmtPKR(codSst)}</span>
                                        </div>
                                      )}
                                    </div>
                                  )}
                                  ...
                                  {netReceivable != null && (
                                    <div className={cn(
                                      ...
                                      netReceivable >= 0 ? "bg-emerald-50 dark:bg-emerald-950/30" : "bg-red-50 dark:bg-red-950/30"
                                    )}>
                                      <span className="font-semibold">Net Receivable</span>
                                      <span className={cn("tabular-nums font-bold",
                                        netReceivable >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-500 dark:text-red-400"
                                      )}>
                                        {fmtPKR(netReceivable)}
                                      </span>
                                    </div>
                                  )}
                                  ...
                                  {!isCod && hasCharges && (
                                    <p className="text-[10px] text-muted-foreground text-center pt-1">
                                      Non-COD · courier charges billed to account
                                    </p>
                                  )}
                                </div>
                              );
          ...
          <CardContent className="p-4 flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {codSummary ? `${codSummary.held.count} COD order${codSummary.held.count !== 1 ? "s" : ""} · held by courier` : "Loading…"}
              </p>
            </div>
                    ...
                    <tr key={o.id}
                      ...
                      <td className="px-2 py-3 text-right tabular-nums text-muted-foreground">
                        {sonicLoading && o.trackingNumber ? (
                          ...
                          const weight = td?.shipping_charges ?? null;
                          if (weight == null) return <span className="text-muted-foreground">—</span>;
                          return fmtPKR(weight + (td?.fuel_surcharge ?? 0) + (td?.gst ?? 0));
                        })()}
                      </td>
                      <td className="px-2 py-3 text-right tabular-nums font-medium">
                        {sonicLoading && o.trackingNumber ? (
                          ...
                          const weight = td?.shipping_charges ?? null;
                          if (weight == null) return <span className="text-muted-foreground">—</span>;
                          const totalCharges = weight + (td?.fuel_surcharge ?? 0) + (td?.gst ?? 0);
                          return fmtPKR(o.total - totalCharges);
                        })()}
                      </td>
