-- Fix COD summary and details to only use rows with a real cod_amount from SONIC.
-- Root cause of wrong numbers: COALESCE(sc.cod_amount, o.total_price) was falling
-- back to o.total_price (GBP) when cod_amount was NULL, mixing currencies.
-- Rule: only count rows where courier = 'sonic' AND cod_amount > 0 (real COD data).
...
-- Details modal: only rows with real COD data from SONIC API (cod_amount > 0).
-- net_receivable = cod_amount - courier charges (all in PKR, no currency mixing).
