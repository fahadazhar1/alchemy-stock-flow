-- Root cause confirmed from portal data:
-- - financial_status = 'paid'  → prepaid (customer paid online), SONIC collected nothing → exclude
-- - financial_status = 'pending' + cod_amount = 0 → SONIC API wrong, real COD = order total → include
-- - financial_status = 'pending' + cod_amount > 0 → SONIC API correct → include
-- Fix: filter OUT financial_status = 'paid', use COALESCE(NULLIF(cod_amount,0), total_price) for rest.
      ...
      'count', COALESCE(SUM(
        CASE
          WHEN sc.courier_status ILIKE '%delivered%'
           ...
           AND (o.financial_status IS NULL OR o.financial_status NOT IN ('paid', 'voided'))
          THEN 1 ELSE 0
        END
      )::int, 0),
      'amount', COALESCE(SUM(
