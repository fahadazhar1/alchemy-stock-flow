-- Courier receivables must include ALL SONIC delivered orders, not just COD.
-- Non-COD (prepaid) orders have cod_amount = 0 but still incur shipping charges,
-- fuel surcharge, and GST that SONIC deducts from our payout.
--
-- Per-order net receivable:
--   net_receivable = COALESCE(cod_amount, 0) - shipping_charges - fuel_surcharge - gst
--
-- COD order  → positive net_receivable (we receive money minus charges)
-- Non-COD    → negative net_receivable (we owe courier for shipping expenses)
-- Dashboard  → SUM(all net_receivables) = true amount SONIC owes us (or we owe them)
--
-- Fix over migration 019: released bucket now also requires courier_status ILIKE '%delivered%'
-- so the summary count/amount matches what get_cod_delivered_orders shows in the modal.
