-- Restore migration 021 logic. Migration 022 was a mistake — reverting to
-- "trust SONIC only" broke all COD orders where SONIC returns cod_amount = 0
-- (which is many orders due to SONIC API unreliability).
--
-- Migration 021 had only 4 edge-case errors. This is the correct approach:
--   financial_status = 'paid'  → prepaid order → cod = 0
--   financial_status != 'paid' → COD order     → COALESCE(NULLIF(cod_amount,0), total_price)
