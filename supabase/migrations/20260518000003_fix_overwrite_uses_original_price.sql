-- When overwrite_existing=true on an already-discounted variant, apply the
-- discount % against the ORIGINAL price (compare_at_price), not the current
-- discounted price. Fixes compounding behaviour in all three pricing functions.
...
BEGIN
  ...
  LOOP
    ...
    -- Use original price as base when overwriting an already-discounted variant
    v_base_price := CASE
