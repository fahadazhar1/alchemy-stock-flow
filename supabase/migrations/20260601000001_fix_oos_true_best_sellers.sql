-- Fix get_oos_last_7_days: previously ranked only among OOS products.
-- Now ranks ALL products by 7-day sales velocity, then surfaces the top quartile
-- that happen to be OOS — so "best sellers" is accurate, not just "any recent sale".
...
AS $$
  WITH
  -- Step 1: sales for ALL products in last 7 days (including in-stock ones)
  all_sales AS (
    SELECT
      oi.product_id,
  ...
  -- Step 2: rank into quartiles — top 25% = best sellers
  ranked_sales AS (
    SELECT *,
      NTILE(4) OVER (ORDER BY units_sold_7d DESC, revenue_7d DESC) AS quartile
    FROM all_sales
  ),
  -- Step 3: current inventory scoped to store
  product_inv AS (
    SELECT
      v.product_id,
  ...
  WHERE pi.total_inv = 0    -- currently OOS
    AND s.quartile   = 1    -- top 25% of sellers this week
