-- Live view for product velocity — computed directly from order_items/orders
-- so data is always current (no batch refresh lag from shopify-sync).
-- Replaces the pre-aggregated product_velocity_metrics table for the UI;
-- the table itself is kept for v_replenishment_candidates.
