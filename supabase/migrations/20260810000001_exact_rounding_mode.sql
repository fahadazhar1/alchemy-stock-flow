-- Add "exact" rounding mode: keeps the discounted price at full 2-decimal
-- precision instead of rounding to a whole unit / .99 / .50 — the rounding
-- was silently distorting the requested discount % on lower-priced items
-- (e.g. a 14% campaign landing between 10-13% depending on item price).
...
BEGIN
  ...
  INSERT INTO public.pricing_campaigns (name, action_type, discount_percent, fixed_price, rounding_mode, overwrite_existing, workflow_status, started_at, pre_campaign_inventory, store_id)
  VALUES (p_campaign_name, 'bulk_discount', p_discount_percent, p_fixed_price, p_rounding_mode, p_overwrite_existing, 'Executed', now(), v_pre_inventory, p_store_id)
  ...
  LOOP
    ...
    ELSIF p_rounding_mode = 'exact' THEN
