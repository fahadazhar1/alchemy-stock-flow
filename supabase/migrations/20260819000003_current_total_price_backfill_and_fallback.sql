-- Found while verifying the 20260819000001/000002 current_total_price fixes:
-- PK (60%), KSA (54%), UAE (71%) of orders have current_total_price = NULL —
-- synced before that column existed (20260803000003) and never touched since
-- (no webhook fired). Any SUM(current_total_price) on these orders was
-- silently COALESCEd to 0, which is worse than the frozen-total_price bug we
-- just fixed. Two-part fix:
--   1. bulk_set_current_total_price — used by the new backfill_current_total_price
--      edge function action to backfill the real value from Shopify.
--   2. Interim safety net: every function/view touched in 000001/000002 now
--      uses COALESCE(current_total_price, total_price) instead of bare
--      current_total_price, so an order that hasn't been backfilled/re-synced
--      yet falls back to its historical total_price (matches this project's
--      own COD RPCs' NULLIF/COALESCE fallback convention) instead of reading
--      as zero revenue.

CREATE OR REPLACE FUNCTION public.bulk_set_current_total_price(p_pairs jsonb)
RETURNS integer
LANGUAGE sql
AS $$
  WITH updated AS (
    UPDATE public.orders o
    SET current_total_price = p.ctp
    FROM jsonb_to_recordset(p_pairs) AS p(id uuid, ctp numeric)
    WHERE o.id = p.id
    RETURNING o.id
  )
  SELECT COUNT(*)::integer FROM updated;
$$;
GRANT EXECUTE ON FUNCTION public.bulk_set_current_total_price(jsonb) TO service_role;

-- get_sales_kpis (20260819000001)
...
AS $$
  ...
  prev AS (
    SELECT total_price, current_total_price
    FROM public.orders
  ...
  SELECT
    COALESCE((SELECT SUM(COALESCE(current_total_price, total_price)) FROM mtd), 0)::numeric  AS revenue_mtd,
    (SELECT COUNT(*) FROM mtd)::bigint                                                       AS orders_mtd,
    COALESCE((SELECT SUM(total_price) FROM mtd
              WHERE financial_status IN ('refunded', 'partially_refunded')), 0)::numeric      AS refunded_revenue,
    (SELECT COUNT(*) FROM mtd
       WHERE financial_status IN ('refunded', 'partially_refunded'))::bigint                  AS refunded_orders,
    COALESCE((SELECT SUM(COALESCE(current_total_price, total_price)) FROM prev), 0)::numeric AS prev_revenue,
...
AS $$
  SELECT
    ...
    COALESCE(SUM(COALESCE(current_total_price, total_price)), 0)::numeric AS revenue,
  ...
  SELECT
    ...
    COALESCE(SUM(COALESCE(current_total_price, total_price)), 0)::numeric,
    COUNT(*)::bigint
  FROM public.orders
  WHERE shopify_created_at >= p_prev_start_iso
    AND shopify_created_at <= p_prev_end_iso
...
AS $$
  ...
  period_orders AS (
    SELECT
      ...
      COALESCE(current_total_price, total_price, 0) AS order_revenue
    FROM public.orders
    WHERE shopify_created_at >= p_start_iso
      AND shopify_created_at <= p_end_iso
      AND cancelled_at IS NULL
...
-- get_discount_usage / get_channel_conversion
...
AS $$
  WITH flagged AS (
    SELECT COALESCE(current_total_price, total_price) AS revenue,
  ...
  SELECT
    ...
    COALESCE(SUM(revenue) FILTER (WHERE is_disc), 0)::numeric
  FROM flagged;
$$;
...
AS $$
  WITH cur AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE cancelled_at IS NULL)
        AS cur_orders,
      SUM(COALESCE(current_total_price, total_price)) FILTER (WHERE cancelled_at IS NULL)
        AS cur_revenue,
      COUNT(*) FILTER (WHERE cancelled_at IS NULL
  ...
  prev AS (
    SELECT
      ...
      SUM(COALESCE(current_total_price, total_price)) FILTER (WHERE cancelled_at IS NULL) AS prev_revenue
    FROM orders
...
-- v_discount_code_performance / v_discount_performance
...
SELECT
  ...
  COALESCE(SUM(COALESCE(o.current_total_price, o.total_price)), 0)         AS total_revenue,
  COALESCE(SUM(o.total_discounts), 0)                             AS total_discount_given,
  ROUND(AVG(COALESCE(o.current_total_price, o.total_price))::numeric, 2)  AS avg_order_value,
...
SELECT
  ...
  COALESCE(AVG(COALESCE(current_total_price, total_price)) FILTER (WHERE total_discounts > 0), 0)              AS avg_discounted_aov,
  COALESCE(AVG(COALESCE(current_total_price, total_price)) FILTER (WHERE COALESCE(total_discounts, 0) = 0), 0) AS avg_full_price_aov,
  COALESCE(SUM(COALESCE(current_total_price, total_price)) FILTER (WHERE total_discounts > 0), 0)              AS discounted_revenue,
  COALESCE(SUM(COALESCE(current_total_price, total_price)) FILTER (WHERE COALESCE(total_discounts, 0) = 0), 0) AS full_price_revenue
FROM orders
...
LANGUAGE sql STABLE SECURITY INVOKER AS $$
  SELECT
    ...
    COUNT(*)::bigint                                                                             AS orders,
    CASE WHEN COUNT(*) > 0
         THEN (COALESCE(SUM(COALESCE(current_total_price, total_price)), 0) / COUNT(*))::numeric
         ELSE 0 END                                                                             AS aov
  FROM public.orders
  WHERE cancelled_at IS NULL
...
LANGUAGE sql STABLE SECURITY INVOKER AS $$
  WITH cur AS (
    SELECT COALESCE(SUM(COALESCE(current_total_price, total_price)), 0) AS revenue, COUNT(*) AS orders
    FROM public.orders
    WHERE cancelled_at IS NULL
      AND (p_from IS NULL OR shopify_created_at >= p_from)
      AND (p_store_id IS NULL OR store_id = p_store_id)
  ...
  prior AS (
    SELECT COALESCE(SUM(COALESCE(current_total_price, total_price)), 0) AS revenue
    FROM public.orders
    WHERE cancelled_at IS NULL
